...
void PhysicalSwitch::handle_packet_in(fluid_msg::of13::PacketIn& packet_in_message) {
	...
	if( packet_in_message.table_id()<=1 ) {
		...
		// Make sure that the metadata field was set
		if( metadata_tlv == nullptr ) {
			BOOST_LOG_TRIVIAL(error) << *this
				<< " received packet in without metadata field";
			return;
		}
		...
		fluid_msg::of13::InPort* in_port = packet_in_message.match().in_port();
		if( in_port != nullptr ) {
			// Rewrite the in port to the virtual in port
			in_port->value(port_map.get_virtual(in_port->value()));
		}
		// Send the message
